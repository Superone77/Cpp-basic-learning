...
namespace n_vector
{
	...
	void TVector::m_convertPolarToRect(double length, double angle)
	{
		m_x = length * cos(angle);
		m_y = length * sin(angle);
	}

	void TVector::m_converRectToPolar(double& length, double &angle) const
	{
	...
	ostream& operator<< (ostream& os, const TVector& vec)
	{
		if (TVector::RECT == vec.getMode()) {
			...
			os << '(' << vec.getLength() << ',' << vec.getAngle() << "бу)";
		}
